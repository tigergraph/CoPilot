...
class MapQuestionToSchemaResponse(BaseModel):
    ...
    target_vertex_ids: Dict[str, List[str]] = Field(description="The dictionary of vertex ids mentioned in the question. If there are no vertex ids mentioned, then use an empty dict. formated in {'vertex_type_1': ['vertex_id_1', 'vertex_id_2'], 'vertex_type_2': ['vertex_id_1', 'vertex_id_2']}")
...
class Node(BaseNode):
    node_type: str = Field(description="Type of the node. Describe what the entity is. Ensure you use basic or elementary types for node labels.\n"
...
class Relationship(BaseRelationship):
    relation_type: str = Field(description="Type of the relationship. Describe what the relationship is. Instead of using specific and momentary types such as "

...
if not LLM_SERVICE.endswith(".json"):
    try:
    ...
    with open(LLM_SERVICE) as f:
        llm_config = json.load(f)

if not DB_CONFIG.endswith(".json"):
    try:
...
async def log_requests(request: Request, call_next):
    ...
    logger.info(
        f"{request.url.path} EXIT request_id={req_id} completed_in={formatted_process_time}ms status_code={response.status_code}"
    )
...
def register_query(
    ...
    for query_info in query_list:
        logger.debug(
            f"/{graphname}/registercustomquery request_id={req_id_cv.get()} registering {query_info.function_header}"
        )
...
def upsert_query(
    ...
    try:
        ...
        for request_info in request_data:
            ...
            logger.debug(
                f"/{graphname}/upsertcustomquery request_id={req_id_cv.get()} upserting document"
            )
...
def delete_query(
    ...
    logger.debug(
        f"/{graphname}/deletecustomquery request_id={req_id_cv.get()} deleting {ids}"
    )
...
def retrieve_docs(
    ...
    logger.debug_pii(
        f"/{graphname}/retrievedocs request_id={req_id_cv.get()} top_k={top_k} question={query.query}"
    )
    res = embedding_store.retrieve_similar(
...
def retrieve_answer(
    graphname,
    query: NaturalLanguageQuery,
    conn: TigerGraphConnection = Depends(get_db_connection),
) -> CoPilotResponse:
    logger.debug_pii(
...
def delete_vdb(
    graphname, index_name, conn: TigerGraphConnection = Depends(get_db_connection)
):
...
def search(
    ...
    if query.method.lower() == "hnswoverlap":
        retriever = HNSWOverlapRetriever(
            embedding_service, get_llm_service(llm_config), conn
        )
        res = retriever.search(
            query.question,

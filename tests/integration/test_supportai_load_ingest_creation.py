...
class TestAppFunctions(unittest.TestCase):
    ...
    def test_create_ingest_invalid_data_source(self):
        ...
        response = self.client.post("/SupportAI/supportai/create_ingest", json={"graphname": "SupportAI", "ingest_config": ingest_config}, auth=(self.username, self.password))
        self.assertEqual(response.status_code, 422)  # Assuming FastAPI returns 422 for validation errors
        # Add more assertions as needed
    ...
    def test_ingest_missing_file_path(self):
        ...
        loader_info = {"load_job_id": "test_job_id", "data_source_id": "test_data_source_id"}
        response = self.client.post("/SupportAI/supportai/ingest", json={"graphname": "SupportAI", "loader_info": loader_info}, auth=(self.username, self.password))
        self.assertEqual(response.status_code, 422)  # Assuming FastAPI returns 422 for validation errors
        # Add more assertions as needed

...
class TestValidateFunctionCall(unittest.TestCase):
    ...
    def test_valid_dynamic_function_call(self):
        ...
        doc2 = Document(page_content="`getVertices(vertexType: str, where: str = '', limit: Union[int, str] = None, sort: str = '')` → dict\nRetrieves vertices of the given vertex type...", metadata={'source': '/Users/luzhou/Documents/github/CoPilot/app/pytg_documents/get_vertices.json', 'seq_num': 1, 'function_header': 'getVertices', 'description': 'Get a sample of vertices', 'param_types': {'vertexType': 'str', 'where': 'str', 'limit': 'Union[int, str]', 'sort': 'str'}, 'custom_query': False})
        doc3 = Document(page_content='`getVerticesById(vertexType: str, vertexIds: Union[int, str, list])` → Union[list, str, pd.DataFrame]\nRetrieves vertices of the given vertex type, identified by their ID...', metadata={'source': '/Users/luzhou/Documents/github/CoPilot/app/pytg_documents/get_vertices_by_id.json', 'seq_num': 1, 'function_header': 'getVerticesById', 'description': 'Get vertex information by vertex ID', 'param_types': {'vertexType': 'str', 'vertexIds': 'Union[int, str, List[Union[int, str]]'}, 'custom_query': False})
    ...
    def test_valid_buildin_function_call(self):
        ...
        doc3 = Document(page_content="`getVertexCount(vertexType: Union[str, list] = '*', where: str = '')` → Union[int, dict]\nReturns the number of vertices of the specified type...", metadata={'source': '/Users/luzhou/Documents/github/CoPilot/app/pytg_documents/get_vertex_count.json', 'seq_num': 1, 'function_header': 'getVertexCount', 'description': 'Get the count of a vertex type, optionally with a where filter', 'param_types': {'vertexType': 'Union[str, List[str]]', 'where': 'str'}, 'custom_query': False})
        retrieved_docs = [doc1, doc2, doc3]
